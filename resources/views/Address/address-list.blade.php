        ...
        <div class="sidemenu_mainBox">
            ...
            <div class="sidemenu_content_mainBox">
                ...
                <div class="section-header">
                    ...
                    <a href="{{ENV('APP_URL')}}add-address?screen_name=add-address">Add new address</a>
                </div>
                @if(isset($showAddressList['data']) && count($showAddressList['data']) > 0)
                <div class="address_listingBox">
                    <form>
                        @foreach($showAddressList['data'] as $addressList)
                                        ...
                                        <div class="addres_type d-flex justify-content-between">
                                            <div class="addressIcon">
                                                <div class="address_img">
                                                    @if ($addressList['type'] == "Home")
                                                    <img alt="home" src="{{asset('assets/images/home_location.png')}}">
                                                    @elseif ($addressList['type'] == "Others")
                                                    <img alt="other"
                                                        src="{{asset('assets/images/other_location.png')}}">
                                                    @elseif ($addressList['type'] == "Work")
                                                    <img alt="work"
                                                        src="{{asset('assets/images/office_location.png')}}">
                                                    @endif
                                                </div>
                                                <div class="addressBox">
                                                    <div class="subtitle">{{$addressList['type']}}</div>
                                                    <address>{{$addressList['house_no']}}</address>
                                                    <a href="tel:+{{$addressList['country_code']}}{{$addressList['receiver_phone']}}"></a>
                                                    <div class="subtitle"><span>Mobile No - {{$addressList['country_code']}}
                                                                {{$addressList['receiver_phone']}}</span></div>
                                                </div>
                                            </div>
                                            <div class="icons">
                                                <a href="#" data-toggle="modal" data-bs-toggle="modal"
                                                    data-bs-target="#editAddressModal"
                                                    onclick="populateForm('{{ $addressList['address_id'] }}')">
                                                    <img alt="edit1" src="{{ asset('assets/images/addedit.png') }}">
                                                </a>
                                                <!--@if ($addressList['type'] != "Home")@endif-->
                                                <div class="remove_address"
                                                    onclick="deleteAddressAlert('{{$addressList['address_id']}}')">
                                                    <img alt="delete" src="{{asset('assets/images/adddelete.png')}}">
                                                </div>
                                                
                                            </div>
                                        </div>
                                        <input type="hidden" id="address_id{{$addressList['address_id']}}"
                                            name="address_id{{$addressList['address_id']}}"
                                            value="{{$addressList['address_id']}}">
                                        <input type="hidden" id="house_no{{$addressList['address_id']}}"
                                            name="house_no{{$addressList['address_id']}}"
                                            value="{{$addressList['house_no']}}">
                                        <input type="hidden" id="landmark{{$addressList['address_id']}}"
                                            name="landmark{{$addressList['address_id']}}"
                                            value="{{$addressList['landmark']}}">
                                        <input type="hidden" id="Uname{{$addressList['address_id']}}"
                                            name="Uname{{$addressList['address_id']}}"
                                            value="{{$addressList['receiver_name']}}">
                                        <input type="hidden" id="number{{$addressList['address_id']}}"
                                            name="number{{$addressList['address_id']}}"
                                            value="{{$addressList['receiver_phone']}}">
                                        <input type="hidden" id="email{{$addressList['address_id']}}"
                                            name="email{{$addressList['address_id']}}"
                                            value="{{$addressList['receiver_email']}}">
                                        <input type="hidden" id="latitude{{$addressList['address_id']}}"
                                            name="latitude{{$addressList['address_id']}}"
                                            value="{{$addressList['lat']}}">
                                        <input type="hidden" id="longitude{{$addressList['address_id']}}"
                                            name="longitude{{$addressList['address_id']}}"
                                            value="{{$addressList['lng']}}">
                                        <input type="hidden" id="society_name{{$addressList['address_id']}}"
                                            name="society_name{{$addressList['address_id']}}"
                                            value="{{$addressList['society_name']}}">
                                        <input type="hidden" id="countryCode{{$addressList['address_id']}}"
                                            name="country_code{{$addressList['address_id']}}"
                                            value="{{$addressList['country_code']}}">
                                        <input type="hidden" id="flagcode{{$addressList['address_id']}}"
                                            name="flagcode{{$addressList['address_id']}}"
                                            value="{{$addressList['dial_code']}}">
                                        <input type="hidden" id="type{{$addressList['address_id']}}"
                                            name="type{{$addressList['address_id']}}" value="{{$addressList['type']}}">
                                        <input type="hidden" id="door_image{{$addressList['address_id']}}"
                                            name="door_image{{$addressList['address_id']}}" value="{{$addressList['doorimage']}}"> 
                                    </div>
                                </div>
                            </label>
                        </div>
                        ...
                        <div class="row align-items-center justify-content-center">
                            <div class="col-lg-6">
                                <div class="data_not_available">
                                    <div class="imageBox">
                                        <img src="{{asset('assets/images/No_data_available.png')}}" alt="empty cart"
                                            class="img-fluid">
                                    </div>
                                </div>
                            </div>
                ...
                contentType: false, // Let browser set it (multipart/form-data)
                  success: function (response) {
                    if(response.success == false){
                        ...
                        window.location.href="{{route('address.list')}}";
