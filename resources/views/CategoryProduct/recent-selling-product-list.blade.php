            ...
            @if(isset($productList['data']) && count($productList['data']) > 0)
              <div id="productList">
                <div class="product_list_box">
                 @include('CategoryProduct.recent-selling-product-list-partial', ['productList' => $productList['data']])
                 </div>
            </div>
            <div class="ploader">
...
<script>
 ...
 // ($(window).scrollTop() + $(window).height() >= $(document).height() - 100) 
    function loadMoreData(page) {
        ...
        var screen_name="{{Request::get('screen_name')}}";
        var name="{{Request::get('name')}}";

        $.ajax({
            url: '{{url("recent-selling-product-list")}}',
            type: "get",
