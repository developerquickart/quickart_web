            ...
            <div class="col-lg-6">
                <div class="position__sticky">
                    <div class="free_deliveryBox">
                        <img src="{{asset('assets/images/freeDelivery.png')}}" alt="lock"
                            style="max-width:30px">
                        <span><strong>Free Delivery</strong>
                            - Shop now & Save!</span>
                    </div>
                    <div class="car_product_list_mainBox">
                        @foreach($productList['data']['data'] as $product)
                        <div class="car_product_list">
                            <div class="ordered_product">
                                ...
                                <div class="order_info">
                                    <div class="row">
                                        <div class="col-8">
                                            <div class="order_product_name">{{$product['title']}}
                                            </div>
                                            <div class="actual_price">AED
                                                <span>{{number_format($product['discount_ord_price'], 2)}}</span>
                                            </div>
                                        </div>
                                    </div>
                                                ...
                                                <div class="sub_btn_mainBox">
                                                    <div class="qtyBox justify-content-end">
                                                        <div class="cart_deletebox"
                                                            onclick="deleteCartItem('{{ $product['trail_id'] }}', 'add')">
                                                            <img
                        ...
                        <div class="missed-items-slider order_detailbox section-padding">
                            ...
                            <div class="schedule_header schedule_box" id="scheduleHeader">
                                <div class="day_box" id="dayBox">
                                    {{ Carbon\Carbon::parse($productList['data']['timeslotsdata'][0]['date'])->isToday() ? 'Today' : (Carbon\Carbon::parse($productList['data']['timeslotsdata'][0]['date'])->isTomorrow() ? 'Tomorrow' : Carbon\Carbon::parse($productList['data']['timeslotsdata'][0]['date'])->format('l')) }}
                                </div>
                                <div class="time_box" id="timeBox">
                                    {{$productList['data']['timeslotsdata'][0]['timeslots'][0]['time_slots']}}
                                </div>
                                <div id="arrowBox" class="arrow_box" title="Click Here">
                                    <img src="{{asset('assets/images/down_arrow.png')}}" alt="arrow">
                                </div>
                            </div>
                            <div id="scheduleDetailsBox" class="schedule_deatils_box trailpack_schedule_box">
                                ...
                                <div class="schedule-list schedule-DateList">
                                    @foreach($productList['data']['timeslotsdata'] as $dateList)
                                    <div class="schedule">
                                        <input type="radio"
                                            id="date_{{ Carbon\Carbon::parse($dateList['date'])->format('Ymd') }}"
                                            name="date" value="{{ $dateList['date'] }}"
                                            data-date="{{ $dateList['date'] }}"
                                            onclick="updateTimeSlots('{{ $dateList['date'] }}')"
                                            {{ $loop->first ? 'checked' : '' }}>
                                        <label
                                            for="date_{{ Carbon\Carbon::parse($dateList['date'])->format('Ymd') }}">
                                            <div class="dateBox">
                                                {{ Carbon\Carbon::parse($dateList['date'])->format('d') }}
                                                </div>
                                                <div class="dayBox">
                                                    {{ Carbon\Carbon::parse($dateList['date'])->isToday() ? 'Today' :(Carbon\Carbon::parse($dateList['date'])->isTomorrow() ? 'Tomorrow' : Carbon\Carbon::parse($dateList['date'])->format('l')) }}
                                                </div>
                                        </label>
                                    </div>
                                    @endforeach
                            ...
                            <table>
                                <tbody>
                                    <tr>
                                        <td>Item Total :</td>
                                        <td><span>
                                                @if ($productList['data']['discount_total_price'] !=
                                                $productList['data']['total_mrp'])
                                                <span class="regular-price">AED
                                                    <span>
                                                        {{number_format($productList['data']['total_mrp'], 2) }}</span></span>
                                                <!-- </p> -->
                                                @endif
                                            </span> AED
                                            {{number_format($productList['data']['discount_total_price'], 2)}}
                                        </td>
                                    </tr>
                                    <tr class="small-text">
                                    ...
                                    <tr class="small-text">
                                        <td>VAT</td>
                                        <td>AED {{number_format($productList['data']['vat'], 2)}}
                                        </td>
                                    </tr>
                                    ...
                                    <tr class="small-text" id='codChargesRow'>
                                        ...
                                        <td id="codCharges">AED
                                            {{number_format($productList['data']['vat'], 2)}}
                                        </td>
                                    </tr>
                                    <tr>
                                        <td>To Pay</td>
                                        <td id="totalPay"><strong>AED
                                                {{number_format($productList['data']['discount_total_price'], 2)}}</strong>
                                        </td>
                                    </tr>
                                </tbody>
                            </table>
                        ...
                        <div class="payment_address_box p-0 m-0">
                            <div class="address_detailsBox change_addressbox">
                                <div class="payment_add_img">
                                    <img src="{{asset('assets/images/home_address.png')}}" alt="lock"
                                        style="max-width:45px">
                                </div>
                                <div class="address_details">
                                    <div class="address_head">
                                        Delivering to <span class="address-type" id="addressType"
                                            style="font-weight: bold;">{{ $productList['data']['lastadd'][0]['type'] ?? ''}}</span>
                                    </div>
                                    <div class="address_location" id="showAddress">
                                        <span
                                            class="loaction">{{$productList['data']['lastadd'][0]['house_no'] ?? ''}}</span>
                                    </div>
                                    <input type="hidden" id="addressId" name="address_id"
                                        value="{{ $productList['data']['lastadd'][0]['address_id'] ?? ''}}">
                                </div>
                            </div>
                                ...
                                <div class="pay_btnbox">
                                    <div class="pay_btn_listing" onclick="quickPayTrilPackApiCall('quick')">
                                        <div class="pay_btn_icon">
                                            <img src="{{asset('assets/images/money.svg')}}" alt="Pay Icon"
                                                class="img-fluid">
                                        </div>
                                    ...
                                    <div class="show-card" id="cardNoSe">
                                        <div class="payment_address_box">
                                            <div class="address_detailsBox">
                                                <div class="payment_add_img">
                                                    <img src="{{asset('assets/images/pay_card.png')}}" alt="lock"
                                                        style="max-width:45px">
                                                </div>
                            ...
                            <div class="pay_btn_wrap new_pay_btn_wrap">
                                <div class="trial_total_box">
                                    ...
                                    <span class="total_amount" id="totalPayD">
                                        <strong>AED{{number_format($productList['data']['discount_total_price'], 2)}}</strong>
                                    </span>
                                </div>
                ...
                <div class="col-lg-12">
                    <div class="section-header">
                        <h5 class="heading-design-h5">Saved Addresses</h5>
                         <a class="add_card_btn" href="{{ url('add-address?addedFrom=trailcart') }}">
                                Add New address</a>
                ...
                <!-- <div class="location_map_MainBox">
                    ...
                    <div class="locate_map_button_box">
                        <div class="locate_map_button">
                            <a href="{{ENV('APP_URL')}}google-map" id="google-map">Locate on Map</a>
                        </div>
                        <div class="locate_map_text" id="locationAddress"></div>
                    </div>
                    <div class="location_textBox">
                        <div class="location_infoBox">
                            <img src="{{asset('assets/images/other-location.svg')}}" width="38" height="38" alt="Other">
...
<script>
let baseTotal1 = parseFloat("{{ $productList['data']['discount_total_price'] ?? 0 }}");
...
@if (!empty($productList['data']) && isset($productList['data']['discount_total_price']))
    baseTotal = parseFloat("{{ $productList['data']['discount_total_price'] }}");
@endif
...
<!-- 
{{-- Time slot selection on date selection...G1 --}}
@if(isset($productList['data']) && is_array($productList['data'])) -->

<script>
let timeSlotsData = @json($productList['data']['timeslotsdata']);
...
document.addEventListener("DOMContentLoaded", function() {
    ...
    addressModal.addEventListener("hidden.bs.modal", function() {
        ...
        if(addedFrom == 'trailcart'){
            window.location.href="{{url('trial-pack-cart')}}";
        }
    });
...
function checkOutTrilPackApiCall() {
    ...
    if (addressID !== "" && addressID !== 'undefined' && siNO !== "" && selectedDate !== "" && selectedTimeSlot !== "") {
        $("#ajaxLoader").show();
        var _token = jQuery('meta[name="csrf-token"]').attr('content');
        var url = "{{ENV('APP_URL')}}checkoutTrialPack";
        ...
        jQuery.ajax({
            ...
            success: function(result) {
                ...
                if (result.success === '1') {
                    ...
                    "{{ENV('APP_URL')}}order-complete?screen=trailpack");
                }
            },
            error: function(xhr, status, error) {
                $("#ajaxLoader").hide();
...
function quickPayTrilPackApiCall(method) {
    ...
    if (addressID !== "" && siNO !== "" && selectedDate !== "" && selectedTimeSlot !== "") {
        $("#ajaxLoader").show();
        var _token = jQuery('meta[name="csrf-token"]').attr('content');
        var url = "{{ENV('APP_URL')}}quickPayTrialPack";
