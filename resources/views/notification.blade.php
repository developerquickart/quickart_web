    ...
    <div class="container-fluid">
        <div class="sidemenu_mainBox">
            ...
            <div class="sidemenu_content_mainBox">
                ...
                @if(isset($showNotificationList['data']['data']) && count($showNotificationList['data']['data']) > 0)
                <div class="notification_listing" id="notificationList">
                    <div class="notification_list faq_listingBox">
                        @foreach($showNotificationList['data']['data'] as $data)
                        <div class="accordion accordion-flush" id="accordion1">
                            <div class="accordion-item">
                                <a href="#notificationList1" class="accordion-header">
                                    <button class="accordion-button collapsed" type="button" data-bs-toggle="collapse"
                                        data-bs-target="#noti_{{ str_replace(' - ', '-', trim($data['title'])) }}"
                                        aria-expanded="false"
                                        aria-controls="noti_{{ str_replace(' - ', '-', trim($data['title'])) }}">
                                        {{$data['title']}}
                                    </button>
                                </a>
                                <div id="noti_{{ str_replace(' - ', '-', trim($data['title'])) }}"
                                    class="accordion-collapse collapse" data-bs-parent="#accordion1">
                                    <div class="accordion-body">
                                        @foreach($data['notification_listing'] as $notificationList)
                                        <div class="notificationBox"
                                            style="background-color: {{ setNotificationBGColor($notificationList['noti_title'], 'bg')  }}; 
                                            border: 0.5px solid {{ setNotificationBGColor($notificationList['noti_title'], 'border') }};">
                                            @if ($notificationList['image']!= null)
                                            <div class="notification_img">
                                                <img src="{{$notificationList['image']}}" alt="Notification">
                                            </div>
                                            @endif
                                            <div class="notification_textbox">
                                                <div class="notification_date">
                                                    {{getTimeAgo($notificationList['created_at'])}}</div>
                                                <div class="notification_heading">{{$notificationList['noti_title']}}
                                                </div>
                                                <div class="notification_text">{{$notificationList['noti_message']}}
                                                </div>
                                            </div>
                                        </div>
                                        @endforeach
